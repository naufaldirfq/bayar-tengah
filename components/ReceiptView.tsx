...
import { Upload, Plus, Check, ChevronDown, ChevronUp, Sparkles, Minus, Pencil, Trash2, ListChecks, Users, X, Split, AlertCircle, Camera } from 'lucide-react';
      ...
      <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white sticky top-0 z-20">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <button
            ...
            className={`text-sm ${receiptData ? 'bg-slate-100 text-slate-700 hover:bg-slate-200' : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'} px-5 py-2.5 rounded-full transition-colors flex items-center gap-2 font-medium`}
          >
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!receiptData && !isAnalyzing && !imageUrl && (
          <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center">
            ...
            <button
              onClick={() => cameraInputRef.current?.click()}
              className="mt-4 text-indigo-600 font-medium hover:underline flex items-center gap-2"
            >
              <Camera className="w-4 h-4" />
              Take a Photo
            </button>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6 max-h-48 overflow-y-auto">
                      {people.map(person => (
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all border ${bulkPeople.includes(person)
                              ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm'
                              : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300 hover:text-indigo-600'}
                                        }`}
                        >
